  ...
  <div class="max-width">
    ...
    <div class="form-container">
      <form id="uploadForm" enctype="multipart/form-data">
        ...
        <label for="update">Обновить существующие элементы?</label>
          ...
          success: function(response) {
            try {
              ...
              if (data.success) {
                ...
                var tableHtml = '<table><thead><tr><th>ID</th><th>Код</th><th>Цена</th><th>Статус обновления</th><th>Ошибка</th><th>Детальная страница</th><th>В админке</th><th>Статус операции</th><th>Сообщение об ошибке</th></tr></thead><tbody>';
                ...
                data.result.forEach(function(item) {
                  tableHtml += '<tr><td>' + (item.ID || '—') + '</td><td>' + item.code + '</td><td>' + item.price + '</td><td>' + item.status + '</td><td>' + (item.error || '—') +
                      ...
                      '" target="_blank">Редактировать</a></td><td>' + item.operation_status + '</td><td>' + (item.error_message || '—') + '</td></tr>';
                });
